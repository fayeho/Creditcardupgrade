...
def index():
    if request.method == "POST":
        ...
        s1 = "The score of the credit card upgarde base on decision tree is " + str(pred1)
        
        model2 = joblib.load("CCU_GB")
        pred2 = model2.predict([[purchases,suppcard]])
        s2 = "The score of the credit card upgarde base on gradient boosting is " + str(pred2)
        
        model3 = joblib.load("CCU_Reg")
        pred3 = model3.predict([[purchases,suppcard]])
        s3 = "The score of the credit card upgarde base on Linear Regression is " + str(pred3)
        
        model4 = joblib.load("CCU_NN")
        pred4 = model4.predict([[purchases,suppcard]])
        s4 = "The score of the credit card upgarde base on Neural Network is " + str(pred4)
        
        model5 = joblib.load("CCU_RF")
        pred5 = model5.predict([[purchases,suppcard]])
        s5 = "The score of the credit card upgarde base on Random Forest is " + str(pred5)
